...
void SimpleThread(int which) {
	...
	for(num = 0; num < 20; num++) {
		...
		z = pthread_barrier_wait(&barrier);
		if(z!=0 && z != PTHREAD_BARRIER_SERIAL_THREAD){
			fprintf(stderr, "error: pthread_barrier_wait, z: %d\n", z);
		}
	}
	val = SharedVariable;
...
int main(int argc, char *argv[]) {
	...
	s = pthread_barrier_init(&barrier, NULL, x);
	if(s!=0){
		fprintf(stderr, "error: pthread_barrier_init, s: %d\n", s);
		return EXIT_FAILURE;
	}
